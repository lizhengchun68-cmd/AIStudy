...
StatusOr<RainflowOutput> RainflowModifiedFourPoint::executeRainflowCounting(
    const RainflowInput& input) {
    ...
    const std::vector<CycleRecord> records =
        runModifiedFourPointPipeline(input.load_history, input.params);
